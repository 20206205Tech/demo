...
def main():
    logger.info("🔌 Đang kết nối tới Supabase PostgreSQL...")
    ...
    try:
        ...
        with psycopg2.connect(env.SUPABASE_DATABASE_URL) as conn:
            ...
            with conn.cursor() as cur:
                logger.info("🔍 Đang truy vấn bảng auth.users để lấy ID...")
                ...
                if not user:
                    logger.error(
                        f"❌ Không tìm thấy người dùng nào với email: {env.EMAIL}"
                    )
                    return

                user_id = user[0]
                logger.success(f"✅ Tìm thấy user_id: {user_id}")

                # 2. Lấy secret từ bảng auth.mfa_factors dựa vào user_id
                logger.info("🔍 Đang truy vấn bảng auth.mfa_factors để lấy secret...")
                cur.execute(
                ...
                if not factor:
                    logger.error(
                        f"❌ Không tìm thấy thông tin MFA cho user_id: {user_id}"
                    )
                    return

                secret = factor[0]
                logger.success("✅ Đã lấy được MFA secret thành công.")

                # 3. Tính toán và in ra mã TOTP trong 5 phút (300 giây)
                totp = pyotp.TOTP(secret)
                logger.info("⏳ Bắt đầu tạo mã TOTP (chạy liên tục trong 5 phút)...")
        ...
        logger.info("⏹️ Đã kết thúc chu trình 5 phút.")
